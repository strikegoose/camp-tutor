#!/usr/bin/env python3
"""Step1b 「和平面」语境终审处置(补充指令A v2):「和平面」不做自动替换——
一半是「X和平面」连词歧义,一半是「𬌗平面」误写。逐处 LLM 终审,误写做片段级修正,全部落处置记录。"""
import json, re, sys
...
def main():
    ...
    prompt = (
        ...
        f"(b) 术语误写——应为「{CANON}」(𬌗=咬合,指咬合平面)。\n"
        "输出 JSON 数组:[{\"id\": 编号, \"verdict\": \"连词歧义\" 或 \"术语误写\", \"reason\": \"一句话\"}]\n"
        "拿不准一律判「连词歧义」(保守,不误改)。\n\n" + "\n".join(lines))
    text, _ = llm.chat("step1b_hepingmian", prompt, max_tokens=8192)
